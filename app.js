    ...
    const uncompletedTasksText = document.querySelector('.header__uncompleted-tasks > span');

    const progressPercent = document.querySelector('.header__progress-bar--percent');
    const completedTasks = document.getElementsByClassName('todo-list__item-name--done');
    ...
    const updateBarAndTasks = () => {
        ...
        uncompletedTasksText.textContent = tasks.length - Number(completedTasks.length);
    ...
    const loadTaskList = () => {
        tasks.forEach(allTasks => {
            ...
            if (allTasks.done === false) {
                ...
                todoItem.classList.add('todo-list__item-container', 'todo-list__item-container--opacity');
                todoItem.innerHTML = ` 
    ...
    const getTasksFromLocalStorage = () => {
        ...
        if (localStorage.getItem('tasks') === "[]") {
            localStorage.setItem('tasks', JSON.stringify([{"task":"Test Task Done","done":true}, {"task":"Test Task","done":false}]));
            tasks.push({task:"Test Task Done",done:true}, {task:"Test Task",done:false});
            loadTaskList();
          } else{
            loadTaskList();
          }
